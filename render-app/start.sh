...
if [ -f "dist/server/index.js" ]; then
  ...
  echo "Содержимое директории server:"
  ls -la server/
  echo "Содержимое директории dist/server (если существует):"
  ls -la dist/server/ 2>/dev/null || echo "Директория dist/server не существует"
  exit 1
fi

# Запускаем приложение
echo "🌐 Запуск веб-сервера из файла: ${SERVER_FILE}"
